...
!-----------------------------------------------------------------------
   ...
   use phys_grid,           only: get_rlat_all_p, get_rlon_all_p ! cuiyz
   use time_manager,        only: get_curr_date                  ! cuiyz
   use spmd_utils,          only: masterproc                     ! cuiyz
   ...
   real(r8) :: clat(pcols), clat_day(pcols) ! cuiyz current latitudes (radians) 
   real(r8) :: clon(pcols), clon_day(pcols) ! cuiyz current longitudes(radians) #_day means rearranged by daylight
   integer  ::  yr, mon, day, tod      ! cuiyz time of day (seconds past 0Z)
   !-----------------------------------------------------------------------
   ! START OF CALCULATION
   !-----------------------------------------------------------------------

   ! Initialize output fields:
   call get_rlat_all_p(lchnk, ncol, clat) ! cuiyz
   call get_rlon_all_p(lchnk, ncol, clon) ! cuiyz
   call get_curr_date(yr, mon, day, tod, 0) ! cuiyz
   !if (masterproc) then
   !  write(iulog,*) 'cuiyz add, clat = ', clat
   !  write(iulog,*) 'cuiyz add, clon = ', clon
   !  write(iulog,*) 'cuiyz add, mon = ', mon, ' day = ', day, ' tod = ', tod
   !end if
   fsds(1:ncol)     = 0.0_r8
   ...
   call CmpDayNite(E_cld(:,pverp-rrtmg_levs+1:pver),  cld(:,1:rrtmg_levs-1), &
        Nday, IdxDay, Nnite, IdxNite, 1, pcols, 1, rrtmg_levs-1)
   call CmpDayNite(clat, clat_day, Nday, IdxDay, Nnite, IdxNite, 1, pcols) ! cuiyz
   call CmpDayNite(clon, clon_day, Nday, IdxDay, Nnite, IdxNite, 1, pcols) ! cuiyz
   ...
   ! Define solar incident radiation cuiyz
   do i = 1, Nday
      solin(i)  = sum(sfac(:)*solar_band_irrad(:)) * eccf * coszrs(i)
      !write(iulog, *) 'clat_day = ', clat_day(i)/3.1415926*180
